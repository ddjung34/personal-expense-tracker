"""
Dashboard 정밀 복제 v9 (Colors & Axis Fixed)
1. 축 눈금(레이블) 표시 명령 명시 (tickLblPos='nextTo')
2. 지출 막대 빨간색 적용 (graphicalProperties.solidFill)
"""
import openpyxl
...
if '📊 Dashboard_v9' in wb.sheetnames: del wb['📊 Dashboard_v9']
ws_new = wb.create_sheet('📊 Dashboard_v9')
...
c1.title = "주요 지표 추이"
c1.y_axis.title = '금액 (원)'
c1.legend.position = "tr" # 범례: Top-Right

# [중요] 축 눈금 표시 설정
# nextTo: 축 옆에 레이블 표시 (기본값일 수 있으나 명시)
c1.y_axis.tickLblPos = "nextTo" 
c1.y_axis.majorTickMark = "cross" # 눈금 마크 표시
...
# [중요] 색상 강제 지정 (수입: 파랑, 지출: 빨강)
# series[0]: 수입, series[1]: 지출
try:
    c1.series[0].graphicalProperties.solidFill = "4472C4" # 엑셀 파랑
    c1.series[0].graphicalProperties.line.solidFill = "4472C4" # 테두리도 파랑
    
    c1.series[1].graphicalProperties.solidFill = "FF0000" # 빨강
    c1.series[1].graphicalProperties.line.solidFill = "FF0000" # 테두리도 빨강
...
# 합계 선: 초록색 (비슷하게)
try:
    c2.series[0].graphicalProperties.line.solidFill = "92D050" 
except: pass
...
# 당부의 말: 저장 경로 확인
output_path = r'c:\Users\JTC7\Desktop\01.Python Project\01.Personal Expense Tracker\01.Document\20251214_Dashboard_v9_color.xlsx'
wb.save(output_path)
