"""
Dashboard 정밀 복제 v11 (Fix: Correct Header Rows)
1. 콤보 차트: 헤더 행 참조 수정 (9 -> 11) -> 범례 이름 자동 해결 ("수입", "지출")
2. 지출 구조 차트: 헤더 행 참조 수정 (37 -> 38) -> 범례 이름 자동 해결
3. 지출 구조 차트: 막대에 "카테고리 이름" 표시 (DataLabels.showCatName = True)
4. Manual Title Assignment 제거 (TypeError 방지)
"""
import openpyxl
...
if '📊 Dashboard_v11' in wb.sheetnames: del wb['📊 Dashboard_v11']
ws_new = wb.create_sheet('📊 Dashboard_v11')
...
# 데이터 (Row 38: Header, Row 39~47: Data)
...
cats_c3 = Reference(ws_new, min_col=3, min_row=39, max_row=47) # Category Names
c3.set_categories(cats_c3)

# 카테고리 이름 표시
c3.dataLabels = DataLabelList()
c3.dataLabels.showVal = True      
c3.dataLabels.showCatName = True  # [핵심]
...
output_path = r'c:\Users\JTC7\Desktop\01.Python Project\01.Personal Expense Tracker\01.Document\20251214_Dashboard_v11_legend.xlsx'
wb.save(output_path)
