"""
Dashboard 정밀 복제 v13 (Fix: Legend Titles & Header Row)
1. 문제 원인: 차트 데이터 범위를 11행(데이터 시작행)부터 잡아서, 범례가 첫 번째 데이터 값(숫자)으로 나옴.
2. 해결: 헤더 행을 10행으로 수정 (min_row=10).
3. 안전장치: 10행의 값을 "수입", "지출", "합계"로 명시적으로 덮어쓰기.
"""
import openpyxl
...
print("=" * 70)
print("Dashboard 정밀 복제 v13 (최종 수정)")
print("=" * 70)
...
if '📊 Dashboard_v13' in wb.sheetnames: del wb['📊 Dashboard_v13']
ws_new = wb.create_sheet('📊 Dashboard_v13')
...
# [헤더 값 강제 보정]
# 상단 테이블 헤더 (10행으로 추정)
ws_new.cell(row=10, column=4).value = "수입"
ws_new.cell(row=10, column=5).value = "지출"
ws_new.cell(row=10, column=6).value = "합계"
...
c1.title = "주요 지표 추이"
c1.legend.position = "tr" # 우상단
c1.x_axis.title = "월"   
c1.y_axis.title = "금액" 
c1.y_axis.tickLblPos = "nextTo" # 축 숫자 표시
c1.y_axis.majorTickMark = "out"

# 데이터 (Row 10: Header, Row 11~23: Data)
# min_row를 10으로 수정!
data_bar = Reference(ws_new, min_col=4, min_row=10, max_col=5, max_row=23)
c1.add_data(data_bar, titles_from_data=True)
cats = Reference(ws_new, min_col=3, min_row=11, max_row=23) # 날짜는 11행부터
...
c2 = LineChart()
# 선 차트 데이터도 10행부터 (헤더 포함)
data_line = Reference(ws_new, min_col=6, min_row=10, max_col=6, max_row=23)
...
output_path = r'c:\Users\JTC7\Desktop\01.Python Project\01.Personal Expense Tracker\01.Document\20251214_Dashboard_v13_solved.xlsx'
wb.save(output_path)
